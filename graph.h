...
namespace graph {
inline std::vector<unsigned> get_k(const Graph &G) {
...
inline size_t get_k(const Graph &G, const size_t i) { return G.A[i].size(); }
} // namespace graph
...
/*  Erdos-Renyi  */
template <class URNG> void make_er(Graph &G, const double p, URNG &rng) {
  auto N = G.A.size();
  auto prob = std::uniform_real_distribution<>(0, 1);
...
template <class URNG> void fix(Graph &G, unsigned iter, URNG &rng) {
...
template <class URNG> void make_rr(Graph &G, const unsigned k, URNG &rng) {
